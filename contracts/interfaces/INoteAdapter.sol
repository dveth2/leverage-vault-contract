...
interface INoteAdapter {
    ...
    function isSupported(uint256 noteTokenId, address currencyToken)
        external
        view
        returns (bool);
    ...
    function getLoanInfo(uint256 noteTokenId)
        external
        view
    ...
    function getLiquidateCalldata(uint256 loanId)
        external
        view
    ...
    /// @return Target address
    /// @return Encoded calldata with selector
    function getUnwrapCalldata(uint256 loanId)
        external
        view
        returns (address, bytes memory);
