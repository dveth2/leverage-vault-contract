...
contract ArcadeV2NoteAdapter is INoteAdapter {
    ...
    function getLoanAssets(
        ...
        if (
            ...
            _vaultFactory.isInstance(
                address(uint160(loanData.terms.collateralId))
            )
        ) {
            // Enumerate vault inventory
            IVaultInventoryReporter.Item[]
                memory items = _vaultInventoryReporter.enumerateOrFail(
                    address(uint160(loanData.terms.collateralId))
                );
            ...
            for (uint256 i; i < items.length; i++) {
                if (
                    items[i].itemType !=
                    IVaultInventoryReporter.ItemType.ERC_721
                ) revert UnsupportedCollateralItem();
                collateralAssets[i] = AssetInfo({
    ...
    function getLiquidateCalldata(
        ...
        return (
            ...
            abi.encodeWithSignature("claim(uint256)", loanId)
        );
    }
    ...
    function isLiquidated(uint256 loanId) external view returns (bool) {
        return
            _loanCore.getLoan(loanId).state == LoanLibrary.LoanState.Defaulted;
    }
