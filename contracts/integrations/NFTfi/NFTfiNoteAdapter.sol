...
interface ISmartNft {
    function loans(
        ...
        external
        view
        returns (address /* loanCoordinator */, uint256 /* loanId */);
...
contract NFTfiNoteAdapter is INoteAdapter {
    ...
    bytes32 public constant SUPPORTED_LOAN_TYPE1 =
        bytes32("DIRECT_LOAN_FIXED_REDEPLOY");
    bytes32 public constant SUPPORTED_LOAN_TYPE2 =
        bytes32("DIRECT_LOAN_FIXED_COLLECTION");
    ...
    function isSupported(
        ...
        (address loanCoordinator, uint256 loanId) = _noteToken.loans(
            noteTokenId
        );
        ...
        // Lookup loan data
        IDirectLoanCoordinator.Loan memory loanData = _directLoanCoordinator
            .getLoanData(uint32(loanId));

        // Validate loan is active
        if (loanData.status != IDirectLoanCoordinator.StatusType.NEW)
            return false;
        ...
        if (
            loanType != SUPPORTED_LOAN_TYPE1 && loanType != SUPPORTED_LOAN_TYPE2
        ) return false;

        // Lookup loan currency token
        (, , , address loanERC20Denomination, , , , , , , ) = loanContract
            .loanIdToLoan(uint32(loanId));
    ...
    function getLoanInfo(
        ...
        // Calculate admin fee
        uint256 adminFee = ((maximumRepaymentAmount - loanPrincipalAmount) *
            uint256(loanAdminFeeInBasisPoints)) / 10000;
    ...
    function isExpired(uint256 loanId) external view returns (bool) {
        ...
        ) = IDirectLoan(loanData.loanContract).loanIdToLoan(uint32(loanId));

        return
            loanData.status == IDirectLoanCoordinator.StatusType.NEW &&
            block.timestamp > loanStartTime + loanDuration;
    }
}
